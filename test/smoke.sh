...
curl -sf -XPOST "${BASE}/api/debug/approvals/simulate" -H 'content-type: application/json' \
  -d "{\"sessionId\":\"${sid}\",\"toolName\":\"Bash\",\"input\":{\"command\":\"git status\"}}" >/dev/null
sleep 0.3

pending=$(curl -sf "${BASE}/api/approvals?status=pending")
n=$(echo "$pending" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).length')
check "approval is parked as pending" "$n" "1"

parked=$(curl -sf "${BASE}/api/health" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).parkedApprovals')
check "gate reports one parked waiter" "$parked" "1"

aid=$(echo "$pending" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8"))[0].id')
res=$(curl -sf -XPOST "${BASE}/api/approvals/${aid}/resolve" -H 'content-type: application/json' \
...
st=$(curl -sf "${BASE}/api/approvals/${aid}" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).status')
...
check "resolving twice is rejected" \
  "$(curl -s -o /dev/null -w '%{http_code}' -XPOST "${BASE}/api/approvals/${aid}/resolve" \
...
n=$(curl -sf "${BASE}/api/approvals?status=pending" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).length')
check "matching call did not park" "$n" "0"

by=$(curl -sf "${BASE}/api/approvals?status=allowed" \
...
hits=$(curl -sf "${BASE}/api/rules" | node -pe "JSON.parse(require('fs').readFileSync(0,'utf8')).find(r=>r.id==='${rid}').hits")
...
n=$(curl -sf "${BASE}/api/approvals?status=abandoned" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).length')
check "orphan was marked abandoned" "$n" "1"
st=$(curl -sf "${BASE}/api/sessions/${sid}" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).status')
...
sleep 2.5
n=$(curl -sf "${BASE}/api/approvals?status=expired" | node -pe 'JSON.parse(require("fs").readFileSync(0,"utf8")).length')
...
check "simulate returns 404 without the flag" \
  "$(curl -s -o /dev/null -w '%{http_code}' -XPOST "${BASE}/api/debug/approvals/simulate" \
